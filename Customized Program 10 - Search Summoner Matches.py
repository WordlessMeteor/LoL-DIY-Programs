        ...
        if name == "":
            ...
            if "errorCode" in info and info["httpStatus"] == 404:
                ...
                if region == "TENCENT":
                    ...
                    folder = "召唤师信息（Summoner Information）\\" + "国服（TENCENT）" + "\\" + platform_TENCENT[client_info["--rso_platform_id"]]["zh_CN"] + "（" + platform_TENCENT[client_info["--rso_platform_id"]]["en_US"] + "）" + "\\" + name
                elif region == "GARENA":
                    platform = platform_GARENA[region]
                    folder = "召唤师信息（Summoner Information）\\" + "竞舞（GARENA）" + "\\" + platform_GARENA[region]["zh_CN"] + "（" + platform_GARENA[region]["en_US"] + "）" + "\\" + name
                else:
                    platform = (platform_RIOT | platform_GARENA)[region]
                    folder = "召唤师信息（Summoner Information）\\" + "外服（RIOT）" + "\\" + (platform_RIOT | platform_GARENA)[region]["zh_CN"] + "（" + (platform_RIOT | platform_GARENA)[region]["en_US"] + "）" + "\\" + name
                message = "正在【在线】查询%s大区召唤师%s（玩家通用唯一识别码：%s）的对局……\n[Online] searching for matches of the summoner %s (puuid: %s) on %s server..." %(platform["zh_CN"], name, puuid, name, puuid, platform["en_US"]) #这里考虑到当程序异常中断时，再次运行该程序，文件中新行会紧跟上次运行的最后一行，不容易区分。所以在字符串最前面加了一个换行符。但是这样的话，在创建文件时，第一行也会变成空行。用户如果觉得不顺眼，可以直接双击日志文件去掉第一行，这样看着舒服一些（Considering when the program 
                ...
                for matchID in range(gameIndexBegin, gameIndexEnd + 1):
                    ...
                    if "errorCode" in game_info:
                        ...
                        if game_info["httpStatus"] == 404:
                            ...
                            print(message)
                            message_save(message, folder, name, "【异常信息】")
                            continue
                        if "500 Internal Server Error" in game_info["message"]:
                            ...
                            while "errorCode" in game_info and "500 Internal Server Error" in game_info["message"] and count <= 3:
                                ...
                                print(message)
                                message_save(message, folder, name, "【异常处理】")
                                game_info = await (await connection.request("GET", "/lol-match-history/v1/games/" + matchID)).json()
                    ...
                    if "errorCode" in game_info:
                        ...
                        message = "对局%s信息获取失败！\nMatch %s information capture failure!" %(matchID, matchID)
                        print(message)
                        message_save(message + "\n" + str(game_info), folder, name, "【异常信息】")
                        print(game_info)
                        continue
                    else:
                        players = []
                ...
                message_save("共找到%d场对局。" %(len(matches_found)), folder, name, "【结果】")
                result = "%s大区的召唤师%s（玩家通用唯一识别码：%s）参与的对局如下：\nMatches that involve summoner %s (puuid: %s) on %s server are as follows:\n%s\n\n" %(platform["zh_CN"], name, puuid, name, puuid, platform["en_US"], str(matches_found))
            ...
            while True:
                ...
                while True:
                    ...
                    if puuid == "":
                        ...
                        for matchID in range(gameIndexBegin, gameIndexEnd + 1):
                            matchID = str(matchID)
                            game_info = requests.get("https://%s-cloud-acs.lol.qq.com/v1/stats/game/%s/%s" %(server.tolower(), server.toupper(), matchID)).json()
                            error_occurred = False
