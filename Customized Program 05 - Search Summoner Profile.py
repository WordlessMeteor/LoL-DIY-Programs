    ...
    for i in range(len(ladders)):
        ...
        for j in range(len(ladder["divisions"])):
            ...
            for k in range(len(division["standings"])):
                ...
                logPrint("[%d/%d][%d/%d][%d/%d]" %(i + 1, len(ladders), j + 1, len(ladder["divisions"]), k + 1, len(division["standings"])), end = "\r")
        else:
            logPrint("已完成。 | Done.")
        ...
        if search_LoL:
            ...
            if len(LoLMatchIDs) > 0:
                ...
                for matchId in LoLMatchIDs:
                    ...
                    timeline_note: str = "" if not (old_LoLMatch_detected and update_unsaved_only) or LoLGame_timeline_export and (not save_all_json or timeline_text_saved) else " (Match timeline skipped)" if not LoLGame_timeline_export else " (Exceptional timeline neglected)"
                ...
                if matchId_str == "":
                    ...
                    if matchId_str == "3":
                        ...
                        if TFTMatchIDs == list():
                            ...
                            TFTHistory_df_all = (await reconstruct_TFTHistory(connection, sgpSession, TFTMatchIDs, current_puuid, queues, TFTAugments, TFTChampions, TFTItems, TFTCompanions, TFTTraits, useAllVersions = True, versionList = bigPatches, locale = language_code, log = log))[0]
                            logPrint("是否一同保存每场对局的信息？（输入任意键保存，否则将只导出对局记录）\nSave each match? (Input anything to save each match, or null to only save the scanned match history)")
                            sort_gameInfo_sync_str: str = logInput()
                            sort_gameInfo_sync: bool = bool(sort_gameInfo_sync_str)
                            if not sort_gameInfo_sync:
