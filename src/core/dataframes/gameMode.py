...
def sort_queue_data(queues: list[dict[str, Any]]) -> pandas.DataFrame:
        ...
        for i in range(len(queue_header_keys)):
            ...
            if i <= 43:
                if i == 3: #对局类型（`category`）
                    to_append: Any = categories[queue["category"]]
                elif i == 8: #游戏选择类别（`gameSelectCategory`）
                    to_append = gameSelectCategories[queue["gameSelectCategory"]]
                elif i == 9: #游戏模式分组（`gameSelectModeGroup`）
                    to_append = gameSelectModeGroups[queue["gameSelectModeGroup"]]
                elif i == 25: #双排最高段位限制（`maxTierForPremadeSize2`）
                    to_append = tiers[queue["maxTierForPremadeSize2"]]
                elif i == 32: #英雄选择模式（`pickMode`）
                    to_append = pickModes[queue["pickMode"]]
                elif i == 33: #队列可用性（`queueAvailability`）
                    to_append = queueAvailability_dict[queue["queueAvailability"]]
                elif i == 42 or i == 43: #上次关闭时间和上次开放时间（`lastToggledOffDate` and `lastToggledOnDate`）
                    subkey = "lastToggledOffTime" if i == 42 else "lastToggledOnTime"
                    standard_time = time.strftime("%Y年%m月%d日%H:%M:%S", time.localtime(queue[subkey] / 1000))
                    to_append = standard_time
                else:
                    to_append = queue[key]
            elif i <= 64:
                if i == 46: #禁用模式（`banMode`）
                    to_append = banModes[queue["gameTypeConfig"]["banMode"]]
                elif i == 55: #游戏类型序号（`typeId`）
                    to_append = queue["gameTypeConfig"]["id"]
                elif i == 59: #英雄选择策略（`typeName`）
    ...
    queue_output_order: list[int] = [12, 33, 19, 15, 7, 29, 5, 6, 22, 55, 3, 41, 2, 8, 9, 10, 0, 59, 14, 16, 17, 46, 32, 42, 43, 30, 25, 23, 27, 4, 31, 28, 26, 58, 39, 24, 37, 38, 18, 40, 53, 63, 48, 52, 1, 64, 49, 11, 47, 57, 62, 34, 35, 51, 56, 44, 45, 61, 50, 65, 66, 67, 13]
