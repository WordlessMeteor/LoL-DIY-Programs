...
import copy, os, unicodedata, pandas, requests, time, json, re, pyperclip
                    ...
                    if matchID == "":
                        ...
                        if matchID == "3":
                            ...
                            for filename in filenames:
                                if filename.startswith("Match Information"):
                                    LoLMatchIDs.append(filename.split("-")[-1][:-4])
                            if LoLMatchIDs == list():
                                print("尚未保存过该玩家的数据！\nYou haven't saved this summoner's matches yet!\n")
                                break
                            else:
                                LoLMatchIDs = list(map(int, LoLMatchIDs)) #正确的对局顺序应当是根据整型对局序号的大小来排列的（The correct order of matches should be according to based on the size of LoLMatchIDs of integer type）
                        ...
                        if not detectMode:
                            ...
                            while True:
                                ...
                                if detect != "" and detect[0] == "0":
                                    print('请输入要查询的对局序号，批量查询对局请输入对局序号列表，批量查询全部对局请输入“3”，退出程序请输入“0”：\nPlease enter the match ID to check. Submit a list containing matchIDs to search in batches. Submit "3" to search the currently stored history in batches. Submit "0" to exit the program.')
                                    break
                                excel_name = "Recently Played Summoners in Match %s.xlsx" %session["gameId"]
