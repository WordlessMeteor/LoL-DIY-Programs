...
async def prepare_data_resources(connection: Connection) -> None:
    ...
    global session, regaliaBanners, challenges, queues, gameQueues, summonerIcons, LoLChampions, titles, wardSkins, championSkins, spells, LoLItems, perks, perkstyles, CherryAugments, TFTAugments, TFTCompanions, TFTCompanions_itemIdMap, TFTTraits, TFTChampions, TFTItems, TFTDamageSkins, TFTMapSkins
    ##旗帜（Regalia banner）
                    ...
                    while True:
                        ...
                        if newGroupName == "":
                            ...
                            if response == None:
                                ...
                                friend_group_df_to_print = friend_group_df.iloc[1:].sort_values(by = "id", ascending = True, ignore_index = True)
                                print(format_df(friend_group_df_to_print)[0], end = "\n\n")
                                log.write(format_df(friend_group_df_to_print, width_exceed_ask = False, direct_print = False)[0] + "\n\n")
            ...
            if len(conversations) > 0:
                ...
                while True:
                    ...
                    if mode == "":
                        ...
                        conversations_to_export: list[dict[str, Any]] = conversations
                    elif mode[0] == "2":
                        if len(conversations) > 0:
                ...
                if situation == "":
                            ...
                            if friend_index_str == "":
                                ...
                                try:
                                    ...
                                    if isinstance(friend_index, int) and friend_index in range(1, len(friend_hovercard_df)):
                                        ...
                                        mTypeDict: dict[str, str] = {"1": "chat", "2": "system", "3": "information", "4": "celebration"}
                    ...
                    while True:
                        ...
                        if pid == "":
                            ...
                            messages: list[dict[str, Any]] = await (await connection.request("GET", f"/lol-chat/v1/conversations/{pid}/messages")).json()
                            if "errorCode" in messages and messages["httpStatus"] == 404:
            ...
            while True:
                ...
                if action == "":
                    ...
                    if len(friends) == 0:
                        ...
                        log.write(format_df(friend_hovercard_df.loc[1:, friend_hovercard_fields_to_print], width_exceed_ask = False, direct_print = False, print_index = True, start_index = 1)[0] + "\n")
                        while True:
                            noteChange_input: str = logInput()
                            if noteChange_input == "":
                                continue
                            else:
                                try:
                                    friend_index: int = int(noteChange_input) - 1
                                except ValueError:
                    ...
                    if mode == "":
                        ...
                        while True:
                            ...
                            if method == "":
                                ...
                                recent_LoLPlayer_df = recent_LoLPlayer_df[(recent_LoLPlayer_df["puuid"] != current_info["puuid"]) & (recent_LoLPlayer_df["puuid"] != "00000000-0000-0000-0000-000000000000")] #邀请玩家，当然指的是不包括自己的人类玩家（Of course, the players invited are human players but not himself / herself）
                                recent_TFTPlayer_df = recent_TFTPlayer_df[(recent_TFTPlayer_df["puuid"] != current_info["puuid"]) & (recent_TFTPlayer_df["puuid"] != "00000000-0000-0000-0000-000000000000")]
                        ...
                        if len(friends) == 0:
                            ...
                            log.write(format_df(friend_group_df_to_printwidth_exceed_ask = False, direct_print = True, print_index = True, start_index = 1)[0] + "\n")
                            while True:
                                invite_str = logInput()
                                if invite_str == "":
                                    continue
                                                ...
                                                if action == "":
                                                    ...
                                                    while True:
                                                        ...
                                                        if mode == "":
                                                            ...
                                                            volume_share: bool = True #表示是否所有小队玩家都改成相同的音量（Represents whether all participants except the user itself are set the same volume）
                                                            logPrint("请选择要修改音量的小队玩家的索引：\nPlease select a participant to change volume:")
                                                            print(format_df(participant_record_df.loc[:, participant_record_fields_to_print], print_index = True)[0])
                                                            log.write(format_df(participant_record_df.loc[:, participant_record_fields_to_print], width_exceed_ask = False, direct_print = False, print_index = True)[0] + "\n")
                                                            while True:
                                                                volumeChange_input: str = logInput()
                                                                if volumeChange_input == "":
                                                                    ...
                                                                    try:
                                                                        player_index: int = int(volumeChange_input)
                                                                    except ValueError:
                                                                        logPrint("您的输入有误！请重新输入。\nERROR input! Please try again.")
                                                                    else:
                                                                        ...
                                                                        if player_index in range(1, len(participant_record_df)):
                                                                            index_got = True
                                                                            volumeChange_indices: list[int] = [player_index]
                                                                            break
                                                        elif mode[0] == "2":
                                                            volume_share = True
                                                            logPrint("是否需要对小队玩家取子集？（输入任意键以开始打草稿，否则直接开始输入小队玩家索引。）\nDo you want to get a subset of the current participant data? (Submit any non-empty string to make a draft, or null to input the participant index directly.)")
                        ...
                        if mode == "":
                            ...
                            while True:
                                ...
                                if unblock_input == "":
                                    ...
                                    try:
                                        player_index: int = int(unblock_input) - 1
                                    except ValueError:
                                        player_summonerName: str = unblock_input
