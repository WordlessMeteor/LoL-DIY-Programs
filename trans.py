...
if language_option == "all":
    language_codes = language_dict["CODE"]
else:
...
for i in range(len(language_codes)):
    ...
    trans_files = [file for file in files_exported_pbe if file.endswith(".json") and file.split("/")[-1].startswith("trans") and (language_cdragon[language_code] in file or language_option == "16" and "en_us" in file)]
    trans_files.sort()
